...
class fieldMonitor:
    ...
    def get_data(self,stock_list,timestamp,i):
        ...
        print Dframe.sort_values(by=['concept1','rate'],ascending=False)[['time','stcid','name','rate','concept1']]
        return Dframe
    ...
    def run(self,condition):
        ...
        tmp_result=self.mongodb.stock.resee_everyday.find_one(self.name)
        stock_list=[]
        self.concept1_list=[]
        self.concept2_list=[]

        for item in tmp_result[self.name.keys()[1]]:
            code = item
            self.concept1_list.append(tmp_result[self.name.keys()[1]][item])
            if code[:2] == '60':
                code = 'sh'+code
            else:
                code = 'sz'+code
...
if __name__=='__main__':
    ...
    if content == 1:
        ...
        argu='record'
    tname = {"date":str(date),argu:{"$exists":True}}
