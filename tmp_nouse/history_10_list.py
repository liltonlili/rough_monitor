...
class Gdata:
    ...
    # write mongo
    # 写入mongo，ZT_stocks, DT_stocks,ZT_num, DT_num
    def P2M(self,Tframe):
        for date in np.unique(Tframe.TRADE_DATE.values):
            try:
                ...
                self.mongodb.stock.ZDT_by_date.update({"date":date},{"$set":dicts},True)
            except Exception,e:
                logging.getLogger().info(e)

    # 根据日期，得到当天四种类型股票的frame
    ## 包含了ZT,DT,PZT,PDT四种type类型的frame
    #     TRADE_DATE    TICKER_SYMBOL  rate       type
    ...
    def fetch_data(self,date):
        ...
        # 0   2016-04-30      601989        10.01      ZT
        # 1   2016-05-01
        (ztframe,dtframe)=get_zdt(adata,'CLOSE_PRICE','ACT_PRE_CLOSE_PRICE')        ##得到当天的涨跌停frame
    ...
    #根据tframe（全数据），得到stock_list中的股票的涨幅
